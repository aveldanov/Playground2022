struct MinHeap {
    ...
    mutating private func heapifyUp() {
      ...
      while hasParent(index) && parent(index) < items[index] { //change > or <
    ...
    mutating private func heapifyDown() {
      ...
      while hasLeftChild(index) {
          ...
          if hasRightChild(index) && rightChild(index) > leftChild(index) { //change
              smallerChildIndex = getRightChildIndex(index)
          }

          if items[index] > items[smallerChildIndex] { // change
              break
          } else {
